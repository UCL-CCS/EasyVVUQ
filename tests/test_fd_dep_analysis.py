...
def sampler():
    ...
    s = uq.sampling.FDSampler(vary=vary, distribution=joint, perturbation=0.05, nominal_value=nominal_value, relative_analysis=False)
    return s

@pytest.fixture
def sampler_cholesky():
    # [x1, x2]
    mu = [0., 0.]
    stddev = [1., 1.]
    cov_xy = 1 - 1e-10
...
def sampler_relative():
    ...
    mu = [0., 0.]
    stddev = [1., 1.]
    cov_xy = 1 - 1e-10 # to avoid singular matrix
    cov = np.array([[stddev[0]**2,cov_xy], [cov_xy,stddev[1]**2]]) # almost perfect correlation
    ...
    s = uq.sampling.FDSampler(vary=vary, distribution=joint, perturbation=0.05, nominal_value=nominal_value, relative_analysis=True)
    return s
...
@pytest.fixture
def data_vectors_cholesky(sampler_cholesky):
    np.random.seed(10000000)
    
    data = {('run_id', 0): [], ('x1', 0): [], ('x2', 0): [],
            ('g', 0): [], ('g', 1): [], ('g', 2): [], ('h', 0): [], ('h', 1): []}
    for run_id, sample in enumerate(sampler_cholesky):
        data[('run_id', 0)].append(run_id)
        data[('x1', 0)].append(sample['x1'])
        data[('x2', 0)].append(sample['x2'])
...
@pytest.fixture
def results_vectors_cholesky(data_vectors_cholesky):
    # Post-processing analysis
    sampler, df = data_vectors_cholesky
    analysis = uq.analysis.FDAnalysis(sampler=sampler, qoi_cols=['g', 'h'])
    results = analysis.analyse(df)
    return results
...
def test_results_vec_cholesky(results_vectors_cholesky):
    assert (isinstance(results_vectors_cholesky, PCEAnalysisResults))
    
    # Different access modes
    assert (results_vectors_cholesky.derivatives_first()['g']['x1'][0] == pytest.approx(results_vectors_cholesky.derivatives_first('g')['x1'][0]))
    assert (results_vectors_cholesky.derivatives_first()['g']['x1'][0] == pytest.approx(results_vectors_cholesky.derivatives_first('g', 'x1')[0]))

    # Numerical values
    assert (results_vectors_cholesky.derivatives_first('g')['x1'][0] == pytest.approx(4.0 + 10.0, abs=1e-3)) # perfect correlation, all sensitivity at x1
    assert (results_vectors_cholesky.derivatives_first('g')['x2'][0] == pytest.approx(0.0, abs=1e-3))
    assert (results_vectors_cholesky.derivatives_first('g')['x1'][1] == pytest.approx(4.0 + 10.0, abs=1e-3))
    assert (results_vectors_cholesky.derivatives_first('g')['x2'][1] == pytest.approx(0.0, abs=1e-3))
    assert (results_vectors_cholesky.derivatives_first('g')['x1'][2] == pytest.approx(4.0 + 10.0, abs=1e-3))
    assert (results_vectors_cholesky.derivatives_first('g')['x2'][2] == pytest.approx(0.0, abs=1e-3))

    # since h[0] = my_func1(p)*2
    assert (results_vectors_cholesky.derivatives_first('h')['x1'][0] == pytest.approx(2*4.0 + 2*10.0, abs=1e-3))
    assert (results_vectors_cholesky.derivatives_first('h')['x2'][0] == pytest.approx(0.0, abs=1e-3))
    # since h[0] = my_func1(p)*4
    assert (results_vectors_cholesky.derivatives_first('h')['x1'][1] == pytest.approx(4*4.0 + 4*10.0, abs=1e-3))
    assert (results_vectors_cholesky.derivatives_first('h')['x2'][1] == pytest.approx(0.0, abs=1e-3))
